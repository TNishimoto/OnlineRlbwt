  ...
  public:
        ...
        std::ifstream &ifs) const noexcept
    {
      ...
      for (uint64_t i = 0; i < this->getLenWithEndmarker() - 1; ++i)
      {
        ...
        if (uc != ch)
        {
          std::cerr << "error: bad expansion at i = " << i << ", (" << ch << ") should be (" << uc << ")"
                    << ", idxM = " << idxM << ", pos = " << pos << std::endl;
          return false;
        }
        pos = drle_.rank(ch, idxM, pos, true);
      }
    ...
    uint64_t write_bwt(std::ofstream &ofs, bool verbose) const noexcept
    {
      unsigned char c = 0;
      ...
      for (uint64_t i = 0; i < this->getLenWithEndmarker(); ++i)
      {
        if (i % step == 0)
        {
          ...
          std::cout << " " << i << " characters processed ...[" << sec << " sec]" << std::endl;
        }
